...
pub fn load<F>(mut loadfn: F) -> functions::{{ ctx_name }} where F: FnMut(&'static str) -> *const c_void {
    ...
    {{ template_utils.protect(command.name) }} ctx.{{ command.name|no_prefix }}.aliased(&ctx.{{ alias[0]|no_prefix }});
    {% endfor %}
    {% endif %}
    {% endfor %}
...
pub fn load<F>(mut loadfn: F) where F: FnMut(&'static str) -> *const c_void {
    unsafe {
        ...
        {% if aliases.get(command.name, [])|length > 0 %}
        {% for alias in aliases.get(command.name, [])|reject('equalto', (command.name, command.index)) %}
        {{ template_utils.protect(command.name) }}{{ template_utils.protect(alias[0]) }} storage::{{ command.name|no_prefix }}.aliased(&storage::{{ alias[0]|no_prefix }});
        {% endfor %}
        {% endif%}
        {% endfor %}
    }
